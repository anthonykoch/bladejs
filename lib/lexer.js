...
const LexerPrototype = {
	...
	getDirectiveToken() {
		...
		const start = lastIndex - str.length;
		...
		if (str[0] === '@' && str[str.length -1] === '(') {
			...
			if (lastParenIndex === -1) {
				const betwixt = this.source.substring(this.position, start);
				const line    = this.line + count(betwixt, '\n');

				this.error('Unclosed directive arguments', { line });
			}
